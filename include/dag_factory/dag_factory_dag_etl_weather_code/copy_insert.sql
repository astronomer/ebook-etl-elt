...
CREATE TEMP TABLE tmp_weather_code_data AS 
SELECT * FROM {schema}.{table} LIMIT 0;

-- Copy data into the temporary table (columns from the weather code data)
COPY tmp_weather_code_data 
(date, day, month, year, weather_code, last_updated, latitude, longitude, elevation, utc_offset_seconds, timezone, timezone_abbreviation)
